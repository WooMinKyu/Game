		...
		<c:if test="${pgList.count > 0}"> 
			<table border="1" width="1000px" cellpadding="0" cellspacing="0" align="center" style="border:1px solid grey"> 
				...
				<c:forEach var="article" items="${articleList}">
					<tr height="40px">
                       <td align="center" style="background-color:#C5C5C5; color:black">
                           ${article.help_num}
                       </td>
                       <td style="padding-left:10px; background-color:#C5C5C5;">
                           <a href="/Game/HelpContent.do?help_num=${article.help_num}&pageNum=${pgList.currentPage}" style="color:black">${article.help_sub}</a>
                       </td>
                       <td align="center" style="background-color:#C5C5C5; color:black"> 
                              ${article.help_nick}
                          </td>
                       ...
                       <td align="center" style="background-color:#C5C5C5; color:black">${article.help_view}</td>
                     </tr>
				</c:forEach>
				</table>
				</c:if>
			...
			<div> <!-- Bottom -->
				...
				<!-- 페이징 처리 -->
					...
					<c:if test="${pgList.startPage > pgList.blockSize}">
						<button onClick="location.href='/Game/HelpList.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}'" class="pBtn">&gt</button>
					</c:if>
					<c:forEach var="i" begin="${pgList.startPage}" end="${pgList.endPage}">
						<c:if test="${pgList.currentPage==i}">
							<button onClick="location.href='/Game/HelpList.do?pageNum=${i}&search=${search}&searchtext=${searchtext}'" style="background-color:white; color:black" class="pBtn">${i}</button>
						</c:if>
						<c:if test="${pgList.currentPage!=i}">
							<button onClick="location.href='/Game/HelpList.do?pageNum=${i}&search=${search}&searchtext=${searchtext}'" class="pBtn">${i}</button>
						</c:if>
						</a>
					</c:forEach>
					<c:if test="${endPage < pageCount}">
						<button onClick="location.href='/Game/HelpList.do?pageNum=${pgList.startPage+pgList.blockSize}&search=${search}&searchtext=${searchtext}'" class="pBtn">&lt</button>
					</c:if>
					</div>
				</center>
			</div> <!-- /bottom -->
