...
$(function() {
	$('#writebtn').click(function() {
		location.href="/Game/CsWriteForm.do";
	})
})
</script>
<body>
		...
		<tr>
			...
			<td>
				<table width="100%"> <!-- Top area -->
					<tr>
						<td colspan="2"><h3>CsBoard</h3></td>
					</tr>
					<tr>
						<% if(session.getAttribute("m_id") != null) {%>
							<td style="float:right; margin-right:10px"><button onclick="location.href='/Game/CsWriteForm.do'">Write</button></td>
						<%}%>
					</tr>
		...
		<c:if test="${pgList.count > 0}"> 
			<table border="1" width="1000px" cellpadding="0" cellspacing="0" align="center" style="border:1px solid grey"> 
				...
				<c:forEach var="article" items="${articleList}">
					<tr height="40px">
                       <td align="center" style="background-color:#5E5E5E">
                           ${article.cs_num}
                       </td>
                       <td style="padding-left:10px; background-color:#5E5E5E;">
			...
			<div> <!-- Bottom -->
				...
				<!-- 페이징 처리 -->
					...
					<c:if test="${pgList.startPage > pgList.blockSize}">
						<button onClick="location.href='/Game/CsAdList.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}'" class="pBtn">&gt</button>
						
					</c:if>
					<c:forEach var="i" begin="${pgList.startPage}" end="${pgList.endPage}">
						<c:if test="${pgList.currentPage==i}">
							<button onClick="location.href='/Game/CsAdList.do?pageNum=${i}&search=${search}&searchtext=${searchtext}'" style="background-color:white; color:black" class="pBtn">${i}</button>
							
						</c:if>
						<c:if test="${pgList.currentPage!=i}">
							<button onClick="location.href='/Game/CsAdList.do?pageNum=${i}&search=${search}&searchtext=${searchtext}'" class="pBtn">${i}</button>
							
						</c:if>
						</a>
					</c:forEach>
					<c:if test="${endPage < pageCount}">
						<button onClick="location.href='/Game/CsAdList.do?pageNum=${pgList.startPage+pgList.blockSize}&search=${search}&searchtext=${searchtext}'" class="pBtn">&lt</button>
			...
			<!-- <button onClick="location.href=''" class="pBtn"></button> -->
